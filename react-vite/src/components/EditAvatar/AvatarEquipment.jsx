...
function AvatarEquipment({ avatar }) {
  ...
  useEffect(() => {
    setMain(
      Object.values(equipment).find((item) => item.id == avatar?.equip_main_id)
    );
    setHeadgear(
      Object.values(equipment).find((item) => item.id == avatar?.equip_head_id)
    );
    setArmor(
      Object.values(equipment).find((item) => item.id == avatar?.equip_armor_id)
    );
  }, [
    equipment,
    avatar?.equip_main_id,
    avatar?.equip_head_id,
    avatar?.equip_armor_id,
  ]);
    ...
    <div className="avatar-equipment-wrapper">
      ...
      <div className="ae_equipped-items-container">
        <div className="ae_equipped-items-section">
          ...
          <img
            className="ae_equipped-items-img"
            src={main?.image_url}
            style={{ maxWidth: '100px' }}
          />
          <div className="ae_equipped-items-name">
            {main?.nickname || main?.name}
          </div>
        </div>
        <div className="ae_equipped-items-section">
          ...
          <img
            className="ae_equipped-items-img"
            src={headgear?.image_url}
            style={{ maxWidth: '100px' }}
          />
          <div className="ae_equipped-items-name">
            {headgear?.nickname || headgear?.name}
          </div>
        </div>
        <div className="ae_equipped-items-section">
